...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      br(),
      # --- Third directory input ---
      textInput("third_dir_path", "Enter Third Directory Path:"),
      actionButton("loadThirdDir", "Load Third Directory"),
      br(),
  ...
  dashboardBody(
    ...
    fluidRow(
      ...
      column(width = 9, id = "image_and_buttons_column",
             fluidRow(
               # --- Change: Now 3 columns for 3 image previews ---
               column(width = 4,
               ...
               column(width = 4,
                      box(width = NULL, id = "third_image_preview_box",
                          title = "Third Image Preview",
                          div(id = "third_image_preview",
                              uiOutput("third_image_preview")
                          )
                      )
               )
             ),
...
server <- function(input, output, session) {
  values <- reactiveValues(
    ...
    third_dir_path = NULL, # --- New for third directory ---
    image_files = NULL,
    second_image_files = NULL,
    third_image_files = NULL, # --- New for third directory ---
    current_index = 1,
  ...
  # --- New: Load third directory logic ---
  observeEvent(input$loadThirdDir, {
    values$third_dir_path <- input$third_dir_path
    if (dir.exists(values$third_dir_path)) {
      values$third_image_files <- list.files(values$third_dir_path, pattern = "\\.(jpg|jpeg|png)$", full.names = TRUE)
    } else {
      showNotification("Invalid third directory path", type = "error")
    }
  })
  ...
  # --- New: Third image preview ---
  output$third_image_preview <- renderUI({
    req(values$third_image_files)
    if (length(values$third_image_files) > 0 && !is.na(values$current_index)) {
      current_file <- values$third_image_files[values$current_index]
      if (file.exists(current_file)) {
        ext <- tools::file_ext(current_file)
        mime_type <- switch(ext,
                            jpg = "image/jpeg",
        ...
        img_data <- base64enc::base64encode(readBin(current_file, "raw", file.info(current_file)$size))
        tagList(
          tags$p(id = "third_image_caption", basename(current_file)),
          tags$img(src = paste0(sprintf("data:%s;base64,", mime_type), img_data), 
                   style = "max-width: 100%; max-height: 100%; object-fit: contain;")
        )
      }
